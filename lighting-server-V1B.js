...
serverCode = serverCode.replace(
  ...
  "const filePath = path.join(__dirname, 'lighting-app-V1B.html');"
);
...
`;
// Add custom fixture library endpoints
const fixtureEndpoints = `
  // --- Save custom fixture to disk ---
  if (req.url === '/api/save-fixture' && req.method === 'POST') {
    let body = '';
    req.on('data', chunk => body += chunk);
    req.on('end', () => {
      try {
        const { name, profile } = JSON.parse(body);
        const fixtureDir = path.join(__dirname, 'custom-fixtures');
  ...
  // --- List custom fixtures from disk ---
  if (req.url === '/api/list-fixtures' && req.method === 'GET') {
    try {
      const fixtureDir = path.join(__dirname, 'custom-fixtures');
      const fixtures = [];
      if (fs.existsSync(fixtureDir)) {
        ...
        for (const file of files) {
          ...
          try {
            ...
            fixtures.push({ name: file.replace('.json', ''), ...data });
          } catch(e) {}
        }
      }
      // Also scan USB drives for .fixture files
      const usbFixtures = [];
      try {
        const internalNames = ['Macintosh HD', 'Preboot', 'Recovery', 'VM', 'Update', 'Data'];
        if (fs.existsSync('/Volumes/')) {
          const volumes = fs.readdirSync('/Volumes/', { withFileTypes: true });
          for (const vol of volumes) {
            if (!vol.isDirectory() || internalNames.includes(vol.name)) continue;
            ...
            const scanDir = (dir, depth) => {
              ...
              try {
                ...
                for (const e of entries) {
                  if (e.name.startsWith('.')) continue;
                  const fp = path.join(dir, e.name);
                  if (e.isFile() && (e.name.endsWith('.fixture') || e.name.endsWith('.json'))) {
                    try {
                      const data = JSON.parse(fs.readFileSync(fp, 'utf8'));
                      if (data.channels && data.map) {
                        usbFixtures.push({ name: e.name.replace(/\\.(fixture|json)$/, ''), path: fp, volume: vol.name, ...data });
                      }
                    } catch(err) {}
                  } else if (e.isDirectory() && depth < 2) {
                    scanDir(fp, depth + 1);
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, fixtures, usbFixtures }));
    } catch (e) {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ok: true, fixtures: [], usbFixtures: [] }));
    }
  ...
  // --- Import fixture from USB path ---
  if (req.url.startsWith('/api/import-fixture/') && req.method === 'GET') {
    const fp = decodeURIComponent(req.url.replace('/api/import-fixture/', ''));
    try {
      if (!fp.startsWith('/Volumes/')) { res.writeHead(403); res.end('Forbidden'); return; }
      const data = fs.readFileSync(fp, 'utf8');
      res.writeHead(200, { 'Content-Type': 'application/json' });
...
// Add WebSocket handlers for dmx_test, ping, subnet_config
const wsHandlers = `
      // --- DMX Test (monitor test ramp / single channel) ---
      if (msg.type === 'dmx_test') {
        const uni = msg.universe || 1;
        if (uni >= 1 && uni <= 64 && msg.data && msg.data.length === 512) {
          // Write test data directly to output buffer and mark dirty
          for (let i = 0; i < 512; i++) {
            outputBuffer[uni][i] = msg.data[i] || 0;
          }
      ...
      if (msg.type === 'ping') {
        ...
        if (targetIP && /^\\d{1,3}(\\.\\d{1,3}){3}$/.test(targetIP)) {
          ...
          const cmd = isWin ? \`ping -n 3 -w 1000 \${targetIP}\` : \`ping -c 3 -W 1 \${targetIP}\`;
          exec(cmd, { timeout: 10000 }, (err, stdout, stderr) => {
            const success = !err && stdout.includes('ttl');
            ws.send(JSON.stringify({ type: 'ping_result', ip: targetIP, success, output: stdout || stderr || (err ? err.message : '') }));
          });
        }
      }

      // --- Subnet config ---
      if (msg.type === 'subnet_config') {
        const mask = msg.mask;
        if (mask && /^\\d{1,3}(\\.\\d{1,3}){3}$/.test(mask)) {
          console.log('[SUBNET]', mask);
          ws.send(JSON.stringify({ type: 'subnet_config_ack', mask }));
        }
      }
`;
...
const m = new Module(path.join(__dirname, 'lighting-server-V1B.js'));
m.filename = path.join(__dirname, 'lighting-server-V1B.js');
m.paths = Module._nodeModulePaths(__dirname);
m._compile(serverCode, path.join(__dirname, 'lighting-server-V1B.js'));
